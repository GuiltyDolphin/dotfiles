...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package evil
    ...
    (customize-set-variable 'evil-undo-system 'undo-tree
                            "Note that for undo-tree must also be enabled for 'undo-tree option to work.")
...
#+BEGIN_SRC emacs-lisp
  (projectile-register-project-type "my-emacs-cask" '("Cask") :project-file "Cask" :compile "make build" :test "make test" :test-suffix "-tests" :test-dir "tests")
#+END_SRC
...
Useful timestamp manipulation.

#+BEGIN_SRC emacs-lisp
  (defun my-org-format-time-as-time-stamp (time &optional with-hm inactive)
  ...
  See `org-insert-time-stamp' for more information."
    (let ((fmt (funcall (if with-hm 'cdr 'car) org-time-stamp-formats)) stamp)
      (when inactive (setq fmt (concat "[" (substring fmt 1 -1) "]")))
      (format-time-string fmt time)))

  (defun my-org-format-time-as-inactive-time-stamp (time &optional with-hm)
    "Format TIME as an inactive timestamp.

  WITH-HM means use the stamp format that includes the time of the day.

  See `my-org-format-time-as-time-stamp' for more information."
    (my-org-format-time-as-time-stamp time with-hm t))
#+END_SRC

=my-org-created-property-name= specifies a property used to
track when entries were created. =my-org--set-created-at= can
be used to add/set the created property for an entry.

#+BEGIN_SRC emacs-lisp
  (defvar my-org-created-property-name "CREATED"
    "Property name used to track when entries were created.")

  (defun my-org--set-created-at (&optional time)
    "Add a property indicating when the current entry was created.

  TIME optionally specifies a time other than now to set the property to."
    (org-set-property my-org-created-property-name (my-org-format-time-as-inactive-time-stamp (or time (current-time)) t)))
#+END_SRC

**** Agenda

#+BEGIN_SRC emacs-lisp
  (defvar my-org-gtd-type-property-name "ORG_GTD_TYPE"
    "Name of the property used to identify the GTD type of Org elements.")

  (defvar my-org-gtd-next-action-property-keyword "next_action"
    "Keyword used with `my-org-gtd-type-property-name' for next actions.")

  (defvar my-org-gtd-waiting-for-property-keyword "waiting_for"
    "Keyword used with `my-org-gtd-type-property-name' for \"waiting for\" actions.")

  (defvar my-org-gtd-project-property-keyword "project"
    "Keyword used with `my-org-gtd-type-property-name' for projects.")

  (defun my-org-join-filters (filter1 filter2)
    "Combine FILTER1 and FILTER2 into a new agenda filter."
    (if (and filter1 filter2) ;; check both filters are specified
        (if (string-match-p "^[+-/&|]" filter2)
            (concat filter1 filter2) ;; if the second filter already has a search connector then just append
          (format "%s+%s" filter1 filter2))
      ;; if both filters aren't specified, give back the first that is
      (or filter1 filter2)))

  (defun my-org-gtd-filter-is-action (&optional filter)
    "Add a condition that the entry matched by FILTER is also a next action.

  If FILTER is not specified, then just provide a filter that matches next actions."
    (my-org-join-filters (format "%s=\"%s\"" my-org-gtd-type-property-name my-org-gtd-next-action-property-keyword) filter))

  (defun my-org-gtd-filter-is-waiting-for (&optional filter)
    "Add a condition that the entry matched by FILTER is also a waiting for.

  If FILTER is not specified, then just provide a filter that matches waiting fors."
    (my-org-join-filters (format "%s=\"%s\"" my-org-gtd-type-property-name my-org-gtd-waiting-for-property-keyword) filter))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun my-org-gtd-filter-is-project (&optional filter)
    "Add a condition that the entry matched by FILTER is also a project.

    If FILTER is not specified, then just provide a filter that matches projects."
    (my-org-join-filters (format "%s=\"%s\"" my-org-gtd-type-property-name my-org-gtd-project-property-keyword) filter))

  (defvar my-org-gtd-match-project
    (my-org-gtd-filter-is-project (format "+FILE=\"%s\"&+CATEGORY=\"project\"" (expand-file-name my-org-gtd-project-file)))
      ...
      ((tags ,my-org-gtd-match-project-complete
       ...
       (tags ,my-org-gtd-match-project-cancelled
             ((org-agenda-overriding-header "Cancelled Projects")
              (org-agenda-files '(,my-org-gtd-project-file))))
       (tags ,(my-org-gtd-filter-is-action "/DONE")
             ((org-agenda-overriding-header "Completed Actions")
              (org-agenda-files '(,my-org-gtd-project-file))))
       (tags ,(my-org-gtd-filter-is-action "/CANCELLED")
             ((org-agenda-overriding-header "Cancelled Actions")
              (org-agenda-files '(,my-org-gtd-project-file))))
       (tags ,(my-org-gtd-filter-is-waiting-for "/DONE")
             ((org-agenda-overriding-header "Completed Waiting Fors")
              (org-agenda-files '(,my-org-gtd-project-file))))
       (tags ,(my-org-gtd-filter-is-waiting-for "/CANCELLED")
             ((org-agenda-overriding-header "Cancelled Waiting Fors")
              (org-agenda-files '(,my-org-gtd-project-file))))
       (todo "DONE"
       ...
       (todo "CANCELLED"
             ((org-agenda-overriding-header "Cancelled Inbox Items")
              (org-agenda-files ',my-org-gtd-inbox-files)))
       (todo "DONE"
             ((org-agenda-overriding-header "Completed Someday Items")
              (org-agenda-files '(,my-org-gtd-someday-file))))
       (todo "CANCELLED"
             ((org-agenda-overriding-header "Cancelled Someday Items")
              (org-agenda-files '(,my-org-gtd-someday-file))))
       (todo "DONE"
             ((org-agenda-overriding-header "Completed Maybe Items")
              (org-agenda-files '(,my-org-gtd-maybe-file))))
       (todo "CANCELLED"
             ((org-agenda-overriding-header "Cancelled Maybe Items")
              (org-agenda-files '(,my-org-gtd-maybe-file))))))
...
#+BEGIN_SRC emacs-lisp
  ...
  TAGS specifies a list of tags to apply to the heading."
    ...
    (save-excursion
      ...
      (my-org--set-created-at)
      (org-set-tags tags)
      (org-set-property my-org-gtd-type-property-name my-org-gtd-next-action-property-keyword)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my-org-gtd--prompt-waiting-for ()
    ...
    (let ((wf-what (read-string "Waiting for what: "))
          (remind-date (org-read-date nil nil nil "Reminder (empty or in past for no reminder): ")))
  ...
  REMIND-DATE specifies the date at which to re-evaluate the waiting for."
    ...
    (save-excursion
      ...
      (let ((now (current-time)) (remind-date-time (org-time-string-to-time remind-date)))
        ;; if the reminder time is now or in the past, we omit it
        (unless (or (time-equal-p remind-date-time now) (time-less-p remind-date-time now))
          (org-schedule nil remind-date)))
        (my-org--set-created-at)
        (org-set-property my-org-gtd-type-property-name my-org-gtd-waiting-for-property-keyword)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (setq org-capture-templates
        `(("t" "Inbox" entry (file ,my-org-gtd-inbox-file)
           ,(concat "* TODO %?\n:PROPERTIES:\n:" my-org-created-property-name ":  %U\n:END:"))
          ("e" "Calendar")
