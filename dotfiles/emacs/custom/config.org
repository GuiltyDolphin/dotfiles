...
#+BEGIN_SRC emacs-lisp
  (customize-set-variable 'org-refile-targets
                          `((,my-org-gtd-general-file :maxlevel . 3)
                            ...
                            (,my-org-gtd-tickler-file :level . 1)))
#+END_SRC
...
Some variables must be customised for =org-caldav= to
recognise the calendars and their files, and the files need to
be added to the agenda for =org-agenda= to recognise them:
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar my-calendar-sync--log-buffer (my-calendar-sync--get-log-buffer)
    "Buffer used for logging information about calendar syncs.")
  ...
  (defun my-calendar-sync--log (msg)
    ...
    (with-current-buffer my-calendar-sync--log-buffer
  ...
  (defun my-calendar-sync-in-background ()
    ...
    (if (my-calendar-sync--sync-in-progress)
        (progn (warn "not syncing calendar: existing sync in progress")
               (my-calendar-sync--log "sync aborted: sync already in progress"))
      (if (--any (-when-let (cbuffer (get-file-buffer it)) (buffer-modified-p cbuffer))
                 org-caldav-files)
          (progn
            (warn "could not sync calendars, some calendar files are modified")
            (my-calendar-sync--log "sync aborted: some calendar files are modified"))
                                   ...
                                   (progn
                                     ...
                                     (let ((temp-buffer-show-function (lambda (buffer))))
                                       (with-output-to-temp-buffer "*calendar sync result*"
                                         (princ (buffer-string)))))
                                 (warn "Calendar sync failed")
                                 (my-calendar-sync--log "sync failed")))))))
#+END_SRC
